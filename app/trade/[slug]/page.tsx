...
function TpTrade({ tp }: { tp: TradePair }) {
  ...
  const assetImg = info?.collectionDetail?.base_info?.image_url || info?.collectionDetail?.base_info?.imageUrl || ''
  const assetName = info?.collectionDetail?.name || info?.collectionDetail?.base_info.name || ''
