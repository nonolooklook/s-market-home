...
export const PriceChart = ({
  ...
  const values = useMemo(
    () => Array.from({ length: 6 }, (_, index) => Math.round(index * step * 100) / 100).reverse(),
    [step],
  )
  ...
  return (
    <div>
      ...
      <div className='flex h-[256px]'>
        <div className={'w-[36px] shrink-0 flex flex-col justify-between pb-6 text-gray-400 text-xs'}>
          {values?.map((v, i) => (
        ...
        <div className={'grow'}>
            ...
            {data?.map((d, i) => {
              ...
              return (
                <div className='flex flex-col items-center gap-1 w-[3.3%] h-full' key={'data_' + i}>
                  ...
                  />
                  <div
                    className={`flex-1 w-full grid grid-cols-2 px-[2px] py-1 rounded-[4px] ${
                      isSelected ? 'bg-orange-300/30' : ''
                    }`}
                    onClick={() => {
                    ...
                    <div className={`flex gap-1 flex-col-reverse cursor-pointer`}>
                      {d.bidAmount > 0 &&
                        Array.from(Array(d.bidAmount)).map((_, i) => (
                          <div className={`h-[3px] w-ful bg-green-400`} key={'bid_' + i} />
                        ))}
                    </div>
                    <div className={'flex flex-col-reverse gap-1 cursor-pointer'}>
                      {d.listAmount > 0 &&
                        Array.from(Array(d.listAmount)).map((_, i) => (
                          <div className={`h-[2px] w-full bg-red-400`} key={'list_' + i} />
                        ))}
                    </div>
                  </div>
                </div>
          ...
          <div className='flex mt-2'>
            {data?.map((d, i) => (
              <div className={'w-[2%] text-center text-[10px] text-gray-400'} key={'xlabel_' + i}>
                {d.price}
              </div>
            ))}
          </div>
